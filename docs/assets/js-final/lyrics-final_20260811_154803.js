...
function parseTimestamp(ts) {
    ...
    // Correct fractional seconds (hundredths or thousandths)
...
async function loadLyricsFile(path) {
    try {
        ...
        const lyrics = lines.map(raw => {
            ...
            const timestamp = tsMatches ? tsMatches[tsMatches.length - 1] : "[0:00]";
...
async function jukeboxLoadLyrics(lyricsPath, audioElement, containerElement) {
    ...
    audioElement.removeEventListener("timeupdate", updateLyricsUIForJukebox);
